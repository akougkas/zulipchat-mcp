...
def install_agent(agent: str, scope: str = "user", directory: Optional[str] = None) -> int:
    ...
    try:
        ...
        if not hasattr(module, "install"):
            logger.error(f"Installer module {installer_module} missing install() function")
            return 2
...
Supported agents:
""" + "\n".join(f"  {agent}: {info['description']}" for agent, info in AGENT_REGISTRY.items())
    )
