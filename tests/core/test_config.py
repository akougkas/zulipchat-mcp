...
class TestConfigManager:
    ...
    def test_env_vars_priority(self, monkeypatch):
        ...
        assert manager.config.debug is False # debug logic: get_debug() if cli_debug is None else cli_debug. So CLI overrides Env for debug.
    ...
    def test_validate_config(self):
        ...
        with patch.dict(os.environ, {"ZULIP_EMAIL": "a", "ZULIP_API_KEY": "b", "ZULIP_SITE": "c"}):
            manager = ConfigManager()
            assert manager.validate_config() is True
    ...
    def test_has_bot_credentials(self):
        ...
        # Env based
        with patch.dict(os.environ, {"ZULIP_BOT_EMAIL": "bot", "ZULIP_BOT_API_KEY": "key"}):
            manager = ConfigManager()
            assert manager.has_bot_credentials() is True
    ...
    def test_get_zulip_client_config(self):
        ...
        # Bot config
        with patch("os.path.exists", return_value=True):
            manager = ConfigManager(config_file="/user/path", bot_config_file="/bot/path")
