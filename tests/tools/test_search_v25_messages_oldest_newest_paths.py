...
async def test_advanced_search_messages_oldest_and_newest(
    ...
    # oldest
    oldest = await advanced_search(
        query="x", search_type=["messages"], sort_by="oldest", limit=2, use_cache=False
    )
    assert oldest["status"] == "success" and oldest["results"]["messages"]["count"] == 3

    # newest
    newest = await advanced_search(
        query="x", search_type=["messages"], sort_by="newest", limit=2, use_cache=False
    )
