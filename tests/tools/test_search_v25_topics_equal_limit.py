...
async def test_topics_has_more_equal_limit(_key, mock_managers, fake_client_class) -> None:
    ...
    class Client(fake_client_class):
        ...
        def get_stream_topics(self, stream_id):  # type: ignore[no-redef]
            return {"result": "success", "topics": [{"name": f"topic{i}"} for i in range(1, limit + 1)]}
    ...
    out = await advanced_search(query="topic", search_type=["topics"], limit=limit, use_cache=False)
