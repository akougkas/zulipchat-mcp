...
from zulipchat_mcp.tools.events_v25 import _active_queues, get_active_queues


def test_get_active_queues_returns_dict() -> None:
    # Ensure at least one fake queue for coverage
    _active_queues.setdefault(
        "q-test",
        {"created_at": 0.0, "lifespan": 10, "client": None, "event_types": ["message"]},
    )
