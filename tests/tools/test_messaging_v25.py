...
from zulipchat_mcp.core import ValidationMode
...
class TestUtilityFunctions:
    ...
    def test_convert_narrow_to_api_format(
        self, sample_narrow_filters: list[dict[str, str]]
    ) -> None:
...
class TestSearchMessagesTool:
    ...
    async def test_search_messages_basic(
        self, mock_managers, sample_messages: list[dict[str, Any]]
    ) -> None:
...
class TestMessagingIntegration:
    ...
    async def test_message_search_edit_workflow(self) -> None:
        ...
        with (
            ...
            mock_identity.execute_with_identity = AsyncMock(
                side_effect=mock_send_execute
            )
            ...
            mock_identity.execute_with_identity = AsyncMock(
                side_effect=mock_search_execute
            )
    ...
    async def test_concurrent_messaging_operations(self) -> None:
        ...
        with (
            ...
            assert len({r["message_id"] for r in results}) == 5  # Unique IDs
    ...
    async def test_error_handling_and_recovery(self) -> None:
        ...
        with (
            ...
            mock_validator.suggest_mode.return_value = ValidationMode.BASIC
            mock_validator.validate_tool_params.side_effect = Exception(
                "Validation error"
            )
...
class TestMessagingPerformance:
    ...
    async def test_large_content_handling(
        self, large_content: str, performance_monitor
    ) -> None:
