...
async def test_stream_analytics_flags_and_errors(mock_managers) -> None:
    ...
    class Client:
        ...
        def get_messages(self, request):  # type: ignore[no-redef]
            ...
            return {"result": "success", "messages": [
                {"id": 1, "content": "hi", "timestamp": now, "sender_full_name": "A", "subject": "t"}
            ]}
    ...
    out = await stream_analytics(stream_name="x", include_message_stats=False, include_user_activity=True, include_topic_stats=True)
