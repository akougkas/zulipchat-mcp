...
class TestMessagingTools:
    ...
    def mock_client(self):
        ...
        client.get_message.return_value = {"result": "success", "message": {"content": "original", "subject": "topic", "display_recipient": "stream"}}
        return client

    @pytest.fixture
    def mock_deps(self, mock_client):
        """Patch ConfigManager and ZulipClientWrapper."""
        with patch("src.zulipchat_mcp.tools.messaging.ConfigManager") as mock_config_cls, \
             patch("src.zulipchat_mcp.tools.messaging.ZulipClientWrapper") as mock_client_cls:
